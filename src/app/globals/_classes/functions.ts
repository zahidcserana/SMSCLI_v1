...
export function formatListPrice(price) {
  if (price) {
    ...
    if (price.hourly_price) {
      return "Start at $" + price.hourly_price.toFixed(2);
    }
    if (price.daily_price) {
      return "Start at $" + price.daily_price.toFixed(2);
    }
    if (price.weekly_price) {
      return "Start at $" + price.weekly_price.toFixed(2);
    }
  }
  return "";
}
